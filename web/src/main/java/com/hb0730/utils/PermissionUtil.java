...
import com.hb0730.domain.RouteMetaVO;
...
public class PermissionUtil {
     ...
     * @return .
     */
    public List<MenuTreeVO> buildBasMenuTree(List<TenantPermissionDto> dtoList, boolean filterButton) {
        if (CollectionUtil.isEmpty(dtoList)) {
            return null;
        }
        List<TenantPermissionDto> tree = buildBasTree(dtoList, filterButton);
        if (CollectionUtil.isEmpty(tree)) {
            return null;
        }
        return _buildBasMenuTree(tree);
    }
     ...
     * @return 树
     */
    private List<TenantPermissionDto> buildBasTree(List<TenantPermissionDto> dataList, boolean filterButton) {
        if (CollectionUtil.isEmpty(dataList)) {
            return null;
        }
        if (filterButton) {
    ...
    private List<MenuTreeVO> _buildBasMenuTree(@Nonnull List<TenantPermissionDto> dtoList) {
        List<MenuTreeVO> menuTree = new ArrayList<>();
        dtoList.forEach(
                item -> {
                    MenuTreeVO tree = new MenuTreeVO();
                    ...
                    tree.setId(item.getId());
                    //子类
                    List<TenantPermissionDto> children = item.getChildren();
                    if (CollectionUtil.isNotEmpty(children)) {
                        tree.setChildren(_buildBasMenuTree(children));
                    }
                    menuTree.add(tree);
                }
        );
